...
SOC_f = 30 # initial energy in battery MWh
...
# Constraints
# Power balance
power_balance = model.addConstrs((gp.quicksum(P_G[i, t] for i in range(5)) + P_pv[t] + P_d[t] == P_c[t] + P_L[t] for t in range(T)), "Power_Balance")
...
model.addConstr(SOC[T-1] >= 100, "SOC_Final")
...
legend = ['Pg1', 'Pg2', 'Pg3', 'Pg4', 'Pg5', 'P_pv', 'P_c', 'P_d']
...
Balance = W_g + W_pv + W_discharge + W_charge + W_load
...
print(f"BESS profits: {BESS_profits:.3f} k Euros")